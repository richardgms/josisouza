...
import { Sparkles, Palette, Leaf } from 'lucide-react';
...
const Services = () => {
    return (
        <section id="servicos" className="py-20 bg-secondary/10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
                    {services.map((category, idx) => (
                        <motion.div
                            ...
                            className="bg-background p-8 md:p-10 rounded-2xl shadow-lg border border-white/50 hover:-translate-y-2 transition-transform duration-500"
                        >
                            ...
                            <div className="space-y-6">
                                {category.items.map((item, index) => (
                                    <div key={index} className="flex flex-col sm:flex-row justify-between items-start group border-b border-secondary/10 pb-4 last:border-0 last:pb-0">
                                        <div className="pr-4 mb-2 sm:mb-0">
                                            ...
                                            <p className="text-sm text-text/60 mt-1 leading-relaxed max-w-xs">{item.description}</p>
                                        </div>
                                        <div className="text-right">
                                            <div className="text-primary font-serif whitespace-nowrap text-lg font-medium">
                                                {item.price}
                                            </div>
                                            {item.subPrice && (
